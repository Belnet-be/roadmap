...
def export_org_projects(org)
  ...
  FileUtils.mkdir_p(cur_dir) unless File.directory?(cur_dir)

  if File.exist?(file_t)
  ...
  begin
    ...
    prev_fn = Dir
              ...
              .map { |f| f.sub(org_dir, '').sub(%r{^/}, '') }
              .sort
              .last
    links = {
      self: uri_base + '/' + cur_fn
    }
    links[:prev] = uri_base + '/' + prev_fn if prev_fn.present?
    ...
    File.delete(ref_file) if File.exist?(ref_file)
...
def project_ld_updated?(project, date_s)
  return true if project[:updated_at] >= date_s
  ...
  project[:plans].each do |plan|
    plan[:sections].each do |section|
      section[:questions].each do |question|
        ...
        return true if answer.present? && answer[:updated_at] >= date_s

        question[:comments].each do |comment|
          return true if comment[:updated_at] >= date_s
        end
      end
    end
  end
  ...
  project[:collaborators].each do |collaborator|
    return true if collaborator[:updated_at] >= date_s
  end
...
def projects_ld(org)
  ...
  while org_admin_plan_ids.size > 0
    ...
    includes = [
      ...
      { roles: { user: %i[identifiers perms] } },
      { template: [
        :org,
        {
          phases: {
...
def cleanup_org_projects(org, max = 30)
  throw 'max should be at least 2' if max < 2
