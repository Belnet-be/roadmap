namespace :ugent do
  ...
  task fix_contributor_email: :environment do
    ...
    contributors.each do |contributor|
      next unless contributor.name.include?('@')
      ...
      if user.present?
        ...
        warn "found contributor email #{email} that is not in user database"
  ...
  task reimport_exported_plans: :environment do
    ExportedPlan.transaction do
      ...
      csv.each do |r|
        ...
        if plan.nil?
          warn 'no plan found with id ' + row['id']
          next
        end
        ...
        if phase.nil?
          warn 'no phase found with title "' + row['phase_title'] + '" from plan id ' + row['id']
          next
        end
        ...
        if user.nil?
          warn 'no user found with id ' + row['user_id'] + ' from plan id ' + row['id']
          next
        end
        ...
        if exported_plan.save
          ...
          warn "unable to save exported_plan: #{exported_plan.errors.full_messages.join(',')}"
        end
      end
  ...
  task move_wayfless_entities: :environment do
    ...
    ActiveRecord::Base.connection.select_all('select * from wayfless_entities').each do |we|
      ...
      id  = Identifier.where(identifiable_type: 'Org', identifiable_id: org.id, identifier_scheme_id: scheme.id,
                             value: we['url'])
                      .first
  ...
  task import_org_id: :environment do
    ...
    csv.each do |r|
      ...
      if scheme.nil?
        ...
        warn "unable to find scheme for name #{row['id_scheme_name']}"
        next
      ...
      if org.nil?
        ...
        warn "unable to find org for abbreviation #{row['org_abbreviation']}"
        next
      ...
      if id.save
        ...
        warn "failed to add/update Identifier to Org #{org.abbreviation}: #{id.errors.full_messages.join(', ')}"
